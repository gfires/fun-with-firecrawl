...
- **Historian confabulation fix** (2026-07-13): on precedent-free questions the historian claimed "no evidence was supplied" though the identical block was given to (and cited by) the other 3 roles — an over-generalization from the L3 system/user split, where the user message never pointed at the system evidence. Fix: a shared anchor line in every role's user message ("The QUESTION and its EVIDENCE are in the system message above…") + de-absolutized historian persona ("no precedent in this evidence" ≠ "no evidence at all"; never claim none was given). Covered by `test/orchestration/committee-messages.test.ts`.
