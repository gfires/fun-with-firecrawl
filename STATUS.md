...
## Replay coherence + single-source cost accounting (2026-07-20)

A recorded run ("AI agent infra", 4 questions) replayed incoherently: the board showed all four questions as "⚡ fault line" with the loop marked done and no reason why, while the header cost ($0.59) didn't match the run's real spend ($0.71). Two distinct bugs, both fixed, both applied to live AND replay (the truth rides the persisted event stream + the one shared reducer).

- **Cost accounting — Tier C single-source drain** (`graph-stream.ts`): `research:usage` emission was forwarded per-node, and the `retrieve` (agentic researcher) and `intake` cases each silently forgot to forward theirs — so ~23% of real, billed spend (33 researcher calls) was in the authoritative cost-tracker total but invisible in the live ticker and per-call table ($0.71 total vs $0.52 of visible line items). Replaced all six per-node forwards with ONE `drainUsages()` that emits a `research:usage` for every call the tracker booked since the last drain (cursor over the append-only `getUsages()`), called after each node and once more after `ensureAnswer`. The display stream now equals the ledger by construction; a final drain also catches calls from a rolled-back super-step (degraded runs). `getUsagesSince(cursor)` planned to avoid the O(n·m) full-array copy (finding open, non-blocking).
- **Authoritative-total reconciliation** (`useResearchStream.ts`): on the terminal `research:mechanics` event the reducer snaps `usage.totalCostUsd` to the tracker's rollup (guarded with `?.` so an old persisted-run replay can't crash). Makes the final displayed cost correct even for a degraded run or an old fixture missing usage events.
- **`convergedReason` on the loop** (`state.ts`, `gate.ts`, `graph.ts`, `research-events.ts`): the gate now reports WHY the loop ended (`cost-headroom` / `no-progress` / `max-loops` / `budget` / `gate-decided-no-retrieve` / `zero-cost-resolved`) on `gate:done`; the board renders a one-line stop-reason banner instead of an unexplained halt.
- **`truncated` GateScore + "answered · gap unchased" verdict** (`gate.ts`, `board.ts`, `QuestionBoard.tsx`, `GateDecisionPanel.tsx`): a question that had a chase-able gap but the run converged before pursuing it is now flagged `truncated` and shown as **answered on the evidence in hand, gap noted** (amber caveat) — distinct from a genuine "⚡ fault line" (retrieval was futile) and never a red failure. Fixes the masquerade where budget-truncated questions read as settled conclusions. The drill-down gate table mirrors it (`RESOLVED*`).
- **Backfill for pre-field traces** (`useResearchStream.ts` `normalizeGateScore` / `deriveConvergedReason`): recorded traces from before these fields existed still light up the banner + verdicts, derived from the gate's (code-generated, stable) reason strings — so EVERY committed/persisted replay is coherent, not just new runs.
- **Loop-0 reconnaissance depth 3 → 4** (`RECON_RESULTS_PER_QUESTION`, `RECON_FLOOR`, both arms): the blind openings sit on one more source per question (floor == ceiling on recon, so loop 0 gathers exactly 4).
- **Canonical stance definition** (`schemas/claim.ts` `STANCE_DEFINITION`): one agent-facing definition of supports/opposes/insufficient, reused by the claim schema `.describe()` and the committee + debate prompt builders (was three subtly-different inline versions). Spells out the field roles misread: stance is the DIRECTIONAL implication for the opportunity, not whether the question was answerable — with a worked example.

tsc clean, 470 vitest green. Reviewed (5-finder high-effort pass, Opus): 3 confirmed correctness findings fixed (unguarded mechanics deref → replay crash; `anyTruncated` scanned only the last gate; drill-down collapsed truncated into RESOLVED); remaining findings are non-blocking cleanup/altitude notes.
